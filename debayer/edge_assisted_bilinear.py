...
def simple_full_bilinear_kernel(top : np.ndarray, bottom : np.ndarray, left : np.ndarray, right : np.ndarray) -> np.ndarray:
...
def resample_r(r : np.ndarray, g_upscaled : np.ndarray, r_coeff : float):
    # Initially resample diagonal (r photodiode)
    g_at_r = bayer_to_rgbg(g_upscaled)[0]

    r_wb = r * r_coeff
    
    diff_r = r_wb - g_at_r

    r_padded = cv2.copyMakeBorder(diff_r, 1,1,1,1, cv2.BORDER_REFLECT)
    r_t = r_padded[1:-1, 1:-1]
    r_b = r_padded[2:,   2:  ]
    r_l = r_padded[2:,   1:-1]
    r_r = r_padded[1:-1, 2:  ]

    diff_r_at_b = simple_delta_mix_bilinear_kernel(r_t, r_b, r_l, r_r)
    diff_r_at_b_padded = cv2.copyMakeBorder(diff_r_at_b, 1,1,1,1, cv2.BORDER_REFLECT)

    r_l = r_padded[1:-1, 1:-1]
    r_r = r_padded[1:-1, 2:]
    r_t = diff_r_at_b_padded[:-2, 1:-1]
    r_b = diff_r_at_b

    diff_r_at_g1 = simple_delta_mix_bilinear_kernel(r_t, r_b, r_l, r_r)

    r_t = diff_r
    r_b = r_padded[2:, 1:-1]
    r_r = diff_r_at_b
    r_l = diff_r_at_b_padded[1:-1, :-2]

    diff_r_at_g2 = simple_delta_mix_bilinear_kernel(r_t, r_b, r_l, r_r)

    return (rgbg_to_bayer(diff_r, diff_r_at_g1, diff_r_at_b, diff_r_at_g2) + g_upscaled) / r_coeff